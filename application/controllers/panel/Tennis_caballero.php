...
class Tennis_caballero extends CI_Controller {
	...
	public function __construct(){
		...
		if(!is_rol_permitido($this->tareas->TAREA_TENNIS_CABALLERO, isset($this->session->rol_actual)?$this->session->rol_actual:-1)){
	...
	public function index()	{
		...
		$this->crear_vista('panel/tennis_caballero', $this->cargar_datos());
	}//end index
	...
	public function cargar_datos(){
		...
		//títulos
		$datos['titulo_seccion'] = 'Inventario de tennis de caballero';

		//información de la BD
		$this->load->model('Tabla_tennis');
		$datos['tennis'] = $this->Tabla_tennis->select_tennis_todos_info_ofertas(0);//genero 0=hombre
	...
	public function crear_vista($nombre_vista=NULL, $datos=NULL){
		...
		$data['menu_lateral_panel'] = crear_menu_lateral_panel($this->tareas->TAREA_TENNIS_CABALLERO, $this->session->rol_actual);
