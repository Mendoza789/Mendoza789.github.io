...
class Usuario_detalles extends CI_Controller {
	...
	public function __construct(){
		...
		if(!is_rol_permitido($this->tareas->TAREA_USUARIO_DETALLES, isset($this->session->rol_actual)?$this->session->rol_actual:-1)){
	...
	public function cargar_datos($id_usuario){
		...
		//títulos
		$datos['titulo_seccion'] = 'Detalles de usuario';

		//información de la BD
		$this->load->model('Tabla_usuarios');
		$datos['info_usuario'] = $this->Tabla_usuarios->select(array('id_usuario'=>$id_usuario))[0];//se obtiene el primero (e idealmente el único) registro obtenido.
	...
	public function crear_vista($nombre_vista=NULL, $datos=NULL){
		...
		$data['menu_lateral_panel'] = crear_menu_lateral_panel($this->tareas->TAREA_USUARIO_DETALLES, $this->session->rol_actual);
